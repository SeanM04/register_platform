...
- the programme AI narrative logic now lives in `dashboard/programmes/ai_insights.py`,
  and it powers the four chart card narratives on `dashboard/programme.html`
...
### Programme dashboard payload

The programme page returns a similar `card_narratives` payload with four narrative cards:
...
In addition, the programme endpoint returns structured diagnostics for UI-level source detection:
...
The `returned_source` field is the authoritative source for frontend AI checks and should be one of `google`, `openai`, or `rules`.

The frontend also checks `card_narratives.source` and only renders the AI badge when the source is `openai` or `google`.
