...
const COMPLETION_DRILLDOWN_MODAL_ID = "home-drilldown-modal";
...
const handleEscapeKey = (event) => {
    if (event.key === "Escape") {
        closeCompletionDrillDownModal();
    }
};

export const isCompletionDrillDownModalOpen = () => Boolean(document.getElementById(COMPLETION_DRILLDOWN_MODAL_ID));

export const closeCompletionDrillDownModal = () => {
    const modal = document.getElementById(COMPLETION_DRILLDOWN_MODAL_ID);
    if (modal) {
        modal.remove();
    }
...
const renderModal = ({ title, subtitle = "", bodyHtml, toneClass = "", onPageChange = null }) => {
    ...
    modal.id = COMPLETION_DRILLDOWN_MODAL_ID;
